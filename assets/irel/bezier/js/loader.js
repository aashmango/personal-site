...
function loadAll() {
  ...
  list.forEach(function(e, idx) {
    ...
    var len = code.length;
    code =  code.map(function(l) { return l.replace(indent,''); }).join("\n");

    var content = code + "\n      draw();";
    content = content + "\n      handleInteraction(getCanvas(), curve).onupdate = function(evt) { reset(); draw(evt); }";
    content = content + "\n      handleInteraction(getCanvas(), curve_2).onupdate = function(evt) { reset(); draw(evt); }";
    content = content + "\n      handleInteraction(getCanvas(), curve_3).onupdate = function(evt) { reset(); draw(evt); }";
    content = content + "\n      handleInteraction(getCanvas(), curve_4).onupdate = function(evt) { reset(); draw(evt); }";
    content = content + "\n      handleInteraction(getCanvas(), curve_5).onupdate = function(evt) { reset(); draw(evt); }";
